...
/**
 * Email sending service for lead expiration notifications
 * Reuses the existing email service functionality
 */
...
export async function sendEmail(
  ...
  try {
    console.log(`=== SENDING EXPIRATION NOTIFICATION EMAIL ===`);
    ...
    const fromName = "Leads Marketplace - URGENT";
    ...
    const effectiveFromAddress = useResendDefault 
      ? "Leads Marketplace - URGENT <onboarding@resend.dev>" 
      : fromAddress;
    ...
    console.log(`Expiration notification email sent successfully to ${redirected ? verifiedEmail + ' (redirected)' : recipient}`);
    ...
    console.error(`=== EXPIRATION EMAIL SENDING FAILED ===`);
    console.error(`Failed to send expiration email to ${recipient}:`, error);
