...
const ProfileContainer = () => {
  ...
  const fetchProfileData = useCallback(async () => {
    try {
      ...
      if (contextUser?.id) {
        console.log("ProfileContainer: Using user from context:", contextUser.id);
        ...
        if (sessionError) {
          ...
          if (loadAttempts >= MAX_RETRY_ATTEMPTS && userData) {
            console.log("ProfileContainer: Using cached user data after multiple failures");
      ...
      const userId = userData?.id || contextUser?.id;
      ...
      if (profileError || !profile) {
        console.warn("ProfileContainer: Error or no profile found:", profileError || "No profile");
